  ...
  return {
    ...
    visitor: {
      ImportDeclaration(p, state) {
        ...
        const sourceFilesOnly = (0, _filter.default)(dirFiles).call(dirFiles, dirFile => !(0, _includes.default)(dirFile).call(dirFile, '.test.'));
        ...
        for (const filePath of sourceFilesOnly) {
          ...
          const parsedPath = _path.default.parse(filePath); // Do it this way to allow double dots e.g. service/payment/payment.utils.ts


          const filePathWithoutExtension = `${parsedPath.dir}/${parsedPath.name}`;
          nodes.push(t.importDeclaration([t.importNamespaceSpecifier(t.identifier(importName + '_' + fileName))], t.stringLiteral(filePathWithoutExtension))); // + <importName>.<fileName> = <importName_fileName>
