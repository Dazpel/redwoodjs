...
const modelFieldToSDL = (field, required = true, types = {}) => {
  ...
  return `${field.name}: ${field.type}${field.isList ? '[]' : ''}${field.isRequired && required ? '!' : ''}`;
};
...
const files = async ({
  ...
  const {
    ...
    relations,
    enums
  } = await sdlFromSchemaModel((0, _pascalcase.default)(_pluralize.default.singular(name)));
...
const builder = yargs => {
  ...
  yargs.positional('model', {
    ...
    type: 'string'
  }).epilogue(`Also see the ${(0, _terminalLink.default)('Redwood CLI Reference', 'https://redwoodjs.com/reference/command-line-interface#generate-sdl')}`);
